...
本レポートでは、ランジュバン方程式を用いたブラウン運動の数値シミュレーションと解析を行う。
...
ここで、$m$は質量、$\gamma$は摩擦係数、$\xi(t)$はノイズである。ランダム力は以下の性質を持つ：
...
正規分布に従う乱数を生成するために、与えられたプログラムによるBox-Muller法を使用した。
...
C言語で実装した粒子運動のシミュレーションプログラムの主要部分を以下に示す：
...
\subsection{課題(2)・(3): 粒子の軌道}

初期条件$r_0 = (0, 0)$、$v_0 = (0, 0)$から開始した粒子の2次元軌道を5回実行し、結果を重ねて表示した（図\ref{fig:trajectories}）。各軌道は異なるランダムウォークを示し、粒子が拡散していく様子が観察される。

\paragraph{物理的考察}
各軌道は、ノイズ$\xi(t)$の影響により異なる経路をたどる。粒子は原点から出発し、ノイズを受けて拡散していく。時間が経過するにつれて、粒子の位置のばらつきが大きくなり、拡散の効果が顕著になる。各実行で異なる軌道が得られることは、粒子運動の確率的性質を反映している。また、2次元空間での拡散により、粒子は原点から離れる方向に移動する傾向がある。

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figures/trajectories_2d.png}
\caption{粒子の2次元軌道（5回実行）}
\label{fig:trajectories}
\end{figure}

\subsection{課題(4): 平均二乗変位の計算}

5回のシミュレーション結果から平均二乗変位を計算し、理論値と比較した（図\ref{fig:msd}）。図には、5回の個別試行の結果を色分けして重ねて表示し、さらに各時間においてこれら5回の平均をとったものも太い黒線で表示している。また、理論値（完全な式）を破線で、拡散極限（$\langle r^2(t) \rangle \approx 4Dt$）を点線で表示している。図から、シミュレーション結果と理論値の間には大きな差が見られる。これは、試行回数が少ない（5回のみ）ことによる統計的なばらつきが大きいためと考えられる。個別の試行結果には大きな統計的なばらつきが見られ、平均をとると理論値に十分近づいても見える。
\begin{figure}[H]
\centering
...
拡散係数$D = k_B T / \gamma$の理論値は、パラメータ$T=1.0$、$m=1.0$、$\gamma=1.0$、$k_B=1.0$の場合、$D = 1.0$となる。ただし、5回の試行のみでは統計的なばらつきが大きく、シミュレーション結果から正確な拡散係数を推定するには試行回数が不足している。
...
図\ref{fig:msd}から、短時間では理論値（完全な式）と拡散極限に差があることが確認できる。これは、粒子が慣性領域から拡散領域へ遷移する過程を示している。ただし、シミュレーション結果は試行回数が少ないため統計的なばらつきが大きく、理論値との定量的な一致は確認できていない。
...
拡散係数は温度$T$に比例する：$D = k_B T / \gamma$。これは、温度が高いほど熱揺らぎが大きくなり、粒子の拡散が促進されることを意味する。温度を$T = 0.5, 1.0, 2.0, 5.0$の4通りに変化させてシミュレーションを実行し、各温度について平均二乗変位の傾きから拡散係数を求めた。図\ref{fig:param_dep}（左）に示すように、シミュレーション結果は理論値$D = k_B T / \gamma$と比較すると、温度が高いほど拡散係数が大きくなるという傾向は確認できるが、試行回数が少ない（5回のみ）ため統計的なばらつきが大きく、理論値との定量的な一致は確認できていない。
...
図\ref{fig:param_dep}（中央）に示すように、理論的には拡散係数は質量に依存せず、$D = k_B T / \gamma$で一定である。しかし、シミュレーション結果では試行回数が少ないため統計的なばらつきが大きく、質量が変化しても拡散係数が一定であるという理論予測を定量的に確認するには至っていない。
...
拡散係数は摩擦係数$\gamma$に反比例する：$D = k_B T / \gamma$。摩擦が大きいほど、粒子の拡散は遅くなる。これは、摩擦が粒子の運動を抑制するためである。摩擦係数を$\gamma = 0.5, 1.0, 2.0$の3通りに変化させてシミュレーションを実行し、各摩擦係数について平均二乗変位の傾きから拡散係数を求めた。図\ref{fig:param_dep}（右）に示すように、摩擦係数を増加させると拡散係数は減少するという傾向は確認できるが、試行回数が少ないため統計的なばらつきが大きく、理論予測$D = k_B T / \gamma$との定量的な一致は確認できていない。
...
\paragraph{物理的考察}
図\ref{fig:energy_theory}から、シミュレーション結果は理論的な指数分布$P(E) = \frac{1}{k_B T} \exp(-E/(k_B T))$と比較すると、指数減衰の傾向が確認できる。これは、熱平衡状態における粒子のエネルギー分布がボルツマン分布に従うことを示唆している。
