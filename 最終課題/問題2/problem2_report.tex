...
\begin{itemize}
    ...
    \begin{equation}
        E_0 = N \varepsilon
    \end{equation}
    \item \textbf{規格化}: $\varepsilon = 1$, $k_B = 1$ として計算
\end{itemize}
...
\begin{equation}
    \ln C(N, M) = \sum_{k=1}^{M} \ln(N - k + 1) - \sum_{k=1}^{M} \ln k
\end{equation}
...
\begin{equation}
    s_{\text{approx}}(x) = -[x \ln x + (1 - x) \ln(1 - x)] \quad (0 < x < 1)
\end{equation}
...
離散データに対して、中心差分法を使用：
\begin{equation}
    \frac{1}{T(M)} \approx \frac{S(M+1) - S(M-1)}{E(M+1) - E(M-1)}
\end{equation}
...
Stirling近似により得られる温度：
\begin{equation}
    T_{\text{th}}(x) = \left[\ln\left(\frac{1-x}{x}\right)\right]^{-1}
\end{equation}
