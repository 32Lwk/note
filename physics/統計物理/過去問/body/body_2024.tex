...
\item 問5：化学ポテンシャルの定義 $\mu_A = (\partial G/\partial N_A)_{T,p,N_B}$ と、理想気体での形 $\mu_A = \mu_A^0(T) + R T \ln(p_A/p^0)$ を用い、終状態では $p_A = x_A p$ なので $\mu_A = \mu_A^0(T) + R T \ln(x_A p/p^0)$ を導く（本文で化学ポテンシャルとは何か・どのように求まるかも詳述）。
\end{enumerate}
...
\subparagraph{化学ポテンシャルとは何か}

\textbf{化学ポテンシャル} $\mu$ は、\textbf{粒子数（またはモル数）を変数として扱うときの「ポテンシャル」}であり、系に粒子を1モル（または1個）追加したときのギブス自由エネルギー $G$ の増分として定義される。多成分系では、成分 $i$ の化学ポテンシャルは
\begin{equation}
\mu_i = \left(\frac{\partial G}{\partial N_i}\right)_{T,\,p,\,N_{j\neq i}}
\end{equation}
である。つまり、\textbf{温度 $T$ と圧力 $p$ を一定に保ったまま}、成分 $i$ のモル数 $N_i$ をわずかに増やしたとき、$G$ がどれだけ増えるかを表す量が $\mu_i$ である。

物理的意味としては次のように理解できる。
\begin{itemize}
\item \textbf{粒子の「逃げやすさ」}：化学ポテンシャルが高い相（または領域）ほど、そこにいる粒子は他へ移りやすい。相平衡では、各成分の化学ポテンシャルが全相で等しくなる（$\mu_A^{\mathrm{左}} = \mu_A^{\mathrm{右}}$ など）。
\item \textbf{拡散の向き}：粒子は化学ポテンシャルが高い方から低い方へ流れる。問3で異種気体を混合するとエントロピーが増大するが、その過程では各成分の化学ポテンシャルが「純粋なとき」より下がり、混合が進む方向と一致する。
\item \textbf{開放系の熱力学}：$T$, $p$ 一定で粒子の出入りがあるとき、平衡条件は $dG = -S\,dT + V\,dp + \sum_i \mu_i\,dN_i = 0$ より、各 $\mu_i$ の釣り合いで決まる。
\end{itemize}

\subparagraph{化学ポテンシャルはどのようにして求まるか}

\begin{enumerate}
\item \textbf{定義から求める}：ギブス自由エネルギー $G(T,p,N_A,N_B,\ldots)$ が与えられていれば、$\mu_A = (\partial G/\partial N_A)_{T,p,N_B}$ を計算すればよい。
\item \textbf{理想気体（単一成分）}：1モルあたりのギブス自由エネルギーは $g = u - Ts + pv$ であり、理想気体では $u$ は $T$ のみ、$pv = RT$、エントロピーは $s = s^0(T) + R\ln(p^0/p)$ のような形で書ける。これらをまとめると、モル化学ポテンシャルは $\mu = \mu^0(T) + RT\ln(p/p^0)$ となる（$\mu^0(T)$ は標準圧力 $p^0$ における化学ポテンシャルで、$T$ のみの関数）。
\item \textbf{理想気体混合物}：各成分は他成分と相互作用しないとみなすと、成分Aの化学ポテンシャルは、\textbf{Aだけが分圧 $p_A$ で占めているときの単一成分の化学ポテンシャル}と同じ形で書ける。すなわち
\begin{equation}
\mu_A = \mu_A^0(T) + R T \ln\frac{p_A}{p^0}.
\end{equation}
ここで $p_A$ は成分Aの\textbf{分圧}（ダルトンの分圧の法則：$p_A = x_A p$、$x_A = N_A/(N_A+N_B)$）である。この式が「どう求まるか」の答である：\textbf{理想気体では $G$ を $N_A$, $N_B$, $T$, $p$ で表し、$(\partial G/\partial N_A)_{T,p,N_B}$ を計算すると、上記の形に帰着する}。あるいは、統計力学では大分配関数から $\mu$ を導入し、理想気体の分配関数から同じ $\mu_A = \mu_A^0(T) + RT\ln(p_A/p^0)$ が導かれる。
\end{enumerate}

\subparagraph{本問での終状態の $\mu_A$}

問3の終状態では、異種の気体AとBが混合し、体積 $V$、温度 $T$、全圧 $p = (N_A+N_B)RT/V$ である。気体Aの分圧はダルトンの分圧の法則より $p_A = N_A R T / V = p \cdot N_A/(N_A+N_B) = x_A p$（$x_A = N_A/(N_A+N_B)$ はモル分率）。したがって理想気体の化学ポテンシャルの式に代入して
\begin{equation}
\mu_A = \mu_A^0(T) + R T \ln\frac{p_A}{p^0}
= \mu_A^0(T) + R T \ln\frac{N_A R T}{V p^0}.
\end{equation}
$V$ を用いずに表すなら $p_A = x_A p$ なので
\begin{equation}
\boxed{\mu_A = \mu_A^0(T) + R T \ln\frac{x_A p}{p^0}}.
\end{equation}

\subparagraph{化学ポテンシャルの物理的意味（本問の文脈）}

$\mu_A = \mu_A^0(T) + R T \ln(p_A/p^0)$ より、\textbf{分圧 $p_A$ が高いほど $\mu_A$ は大きい}。混合後の終状態では、気体Aは全圧 $p$ のうち分圧 $p_A = x_A p$ で存在する。$x_A < 1$ なので $p_A < p$ であり、\textbf{純粋なAだけが圧力 $p$ で存在するときより、混合状態では $\mu_A$ は低くなる}。これは「混合によってAの化学ポテンシャルが下がり、拡散・混合が進む」という直感と一致する。また、相平衡（例えば液体と気体の平衡）では、同じ成分の化学ポテンシャルが両相で等しくなるので、$\mu_A$ を求めることは平衡論でも重要である。
...
\paragraph{熱力学の基本関係式とは（導出と直感的な理解）}
問2で用いる $T\,dS = dE + p\,dV$（内部エネルギーを $E$ で書いているが $U$ と同じ）は\textbf{熱力学の基本関係式}である。どのようにして得られるか、また直感的にどう理解すればよいかをまとめる。

\subparagraph{どのようにして求まるか（導出）}
\begin{enumerate}
\item \textbf{熱力学第一法則}：系が受け取る熱を $\delta Q$、系が外界にする仕事を $\delta W$ とすると、内部エネルギー $U$ の変化は $dU = \delta Q - \delta W$ である。体積変化のみで仕事が $p\,dV$ のとき（準静的な圧力仕事）は $dU = \delta Q - p\,dV$。
\item \textbf{エントロピーと可逆熱}：熱力学第二法則により、\textbf{可逆過程}では熱のやりとりは $\delta Q_{\mathrm{rev}} = T\,dS$ と書ける（クラウジウスの等式）。ここで $S$ は状態量であるエントロピー、$T$ は絶対温度である。
\item \textbf{2つを組み合わせる}：可逆過程を考えると $\delta Q = T\,dS$ なので、第一法則 $dU = \delta Q - p\,dV$ に代入して
\begin{equation}
dU = T\,dS - p\,dV \quad \Longleftrightarrow \quad T\,dS = dU + p\,dV.
\end{equation}
これが\textbf{熱力学の基本関係式}（単純な閉じた系、体積仕事のみの場合）である。$U$, $S$, $V$ はすべて状態量なので、この式は「状態の微小変化どうしの関係」を表しており、可逆過程に限らず成り立つ。
\end{enumerate}

\subparagraph{直感的にどう理解するか}
\begin{itemize}
\item \textbf{「熱」をエントロピーで書き直した第一法則}：$T\,dS = dU + p\,dV$ は「系に流入する可逆熱 $T\,dS$ が、内部エネルギーの増加 $dU$ と、系が外界にする仕事 $p\,dV$ に使われる」と読める。つまり第一法則「エネルギー保存」を、熱 $\delta Q$ の代わりに $T\,dS$ で表した形である。
\item \textbf{なぜ $dS$ を使うか}：熱 $\delta Q$ は経路に依存するが、エントロピー $S$ は状態量なので、$dU = T\,dS - p\,dV$ は「$U$ の変化は $S$ と $V$ の変化だけで一意に決まる」という形になっており、偏微分や積分がしやすい。この式から $(\partial U/\partial S)_V = T$、$(\partial U/\partial V)_S = -p$ などが得られ、温度や圧力が $U(S,V)$ の微分として定義される。
\item \textbf{各項の意味}：$T\,dS$ は「温度 $\times$ エントロピー変化」＝可逆に加えた熱の量。$dU$ は内部エネルギー（分子の運動エネルギーや位置エネルギーなど）の増加。$p\,dV$ は系がピストンなどを押して外界にした仕事。したがって「加えた熱のうち、一部は内部に蓄えられ、残りは仕事として外に出る」という日常的なイメージと対応する。
\item \textbf{定積・定温での読み方}：体積一定（$dV=0$）なら $T\,dS = dU$ なので、熱はすべて内部エネルギーに。温度一定なら $dU$ と $p\,dV$ の関係が $(\partial U/\partial V)_T$ などとして現れ、問2ではこの $T$ 一定のときの係数比較で $p$ を求めている。
\end{itemize}
