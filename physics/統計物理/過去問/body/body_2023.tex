...
\renewcommand{\theHsection}{2023main.\arabic{section}}
\renewcommand{\theHsubsection}{2023main.\arabic{section}.\arabic{subsection}}
...
\item 同じ初期状態 $\{(T_A,V_A),(T_B,V_B)\}$ から出発し、最終的に温度が等しくなるような操作 $\{(T_A,V_A),(T_B,V_B)\} \to \{(T,V_A),(T,V_B)\}$ は、問1の操作だけではない。操作の途中で、外部から仕切り壁を左右に動かすことができるようにしたり、仕切り壁を断熱壁から透熱壁に交換する（あるいはその逆）ことを自在にできるようにすれば、操作次第で、終状態の温度を問1の $T$ より低くもできるし、高くもできる。ただし、終状態の左右の部屋の体積 $V_A$ と $V_B$ は初期状態と同じであり、また操作を通して左右の部屋の間で気体の移動もできないものとする。この操作で達成することができる\textbf{最低温度 $T_{\mathrm{min}}$} を求めよ。
\item 問4でどのような操作をすれば、$T_{\mathrm{min}}$ が達成できるか、操作方法を具体的に述べよ。
\item 問3とは逆に、達成することができる最高温度 $T_{\mathrm{max}}$ を求めよ。またどのような操作をすれば、$T_{\mathrm{max}}$ が達成できるか、操作方法を具体的に述べよ。
\end{enumerate}
...
孤立系では全内部エネルギーが保存するので、問1の終温度 $T$ はエネルギー保存から決まる。問2はエントロピー変化、問3はその正性の証明。問4〜問6は「仕切り壁の操作で達成できる終温度の範囲」を求める発展である。
...
\item 問4：可逆操作では $S_{\mathrm{f}}(T) \ge S_{\mathrm{i}}$ が成り立つ。$S_{\mathrm{f}}(T)$ は $T$ の増加関数なので、等号 $S_{\mathrm{f}}(T)=S_{\mathrm{i}}$ を満たす最小の $T$ が $T_{\mathrm{min}}$。
\item 問5：問4の $T_{\mathrm{min}}$ を達成する具体的な操作方法を述べる。
...
= cR \ln\frac{T}{T_A} + cR \ln\frac{T}{T_B}
= cR \ln\frac{T^2}{T_A T_B}.
\end{equation}
問1の $T = (T_A+T_B)/2$ を代入して
\begin{equation}
\boxed{\Delta S = cR \ln\frac{(T_A+T_B)^2}{4 T_A T_B}}.
\end{equation}

\paragraph{問3：$\Delta S > 0$ の証明}

$\Delta S = cR \ln\bigl((T_A+T_B)^2/(4 T_A T_B)\bigr) > 0$ を示すには、対数の中身が $1$ より大きいこと、すなわち $(T_A+T_B)^2 > 4 T_A T_B$ を示せばよい。分子と分母の差を計算すると
\begin{equation}
(T_A+T_B)^2 - 4 T_A T_B = T_A^2 + 2 T_A T_B + T_B^2 - 4 T_A T_B = T_A^2 - 2 T_A T_B + T_B^2 = (T_A - T_B)^2 \ge 0.
\end{equation}
$T_A \neq T_B$ のとき $(T_A - T_B)^2 > 0$ なので $(T_A+T_B)^2 > 4 T_A T_B$ が成り立ち、したがって $\Delta S > 0$ である。

物理的には、熱が高温から低温へ流れる過程は不可逆であり、熱力学第二法則から孤立系のエントロピーは増大するので $\Delta S > 0$ となる。

\paragraph{問4：最低温度 $T_{\mathrm{min}}$}

\textbf{なぜ $T_{\mathrm{min}}$ が決まるか：}系は孤立しており、どの操作でも全内部エネルギーは保存する。しかし「達成できる終温度 $T$ の下限」は\textbf{熱力学第二法則（エントロピー増大則）}で決まる。孤立系では $\Delta S \ge 0$ が成り立ち、可逆操作では等号、不可逆操作では不等号となる。

終状態のエントロピー $S_{\mathrm{f}}(T) = R \ln(T^c V_A) + R \ln(T^c V_B) + \mathrm{const}$ は $T$ の増加関数なので、$T$ を下げるほど $S_{\mathrm{f}}$ は減少する。したがって、\textbf{等号 $S_{\mathrm{f}}(T)=S_{\mathrm{i}}$ を満たす最小の $T$} が達成可能な最低温度 $T_{\mathrm{min}}$ である。これより低い $T$ だと $S_{\mathrm{f}} < S_{\mathrm{i}}$ となり、第二法則に反するため達成できない。

$S_{\mathrm{f}} - S_{\mathrm{i}} = cR \ln(T^2/(T_A T_B)) \ge 0$ より $\ln(T^2/(T_A T_B)) \ge 0$、すなわち $T^2/(T_A T_B) \ge 1$。$T$, $T_A$, $T_B > 0$ なので $T^2 \ge T_A T_B$、したがって $T \ge \sqrt{T_A T_B}$。等号は可逆過程で達成されるので
\begin{equation}
\boxed{T_{\mathrm{min}} = \sqrt{T_A T_B}}.
\end{equation}
（$T_A$ と $T_B$ の幾何平均。算術平均 $(T_A+T_B)/2$ より小さく、可逆操作を駆使すれば問1より低い終温度を実現できる。）

\paragraph{問5：$T_{\mathrm{min}}$ を達成する操作}

$T_{\mathrm{min}}$ を達成するには、全エントロピーを変えない（$\Delta S = 0$）可逆操作が必要である。具体的には、\textbf{断熱壁を保ったまま仕切り壁を動かし、左室を断熱膨張・右室を断熱圧縮させる}操作を行う。

\textbf{なぜこの操作で $T_{\mathrm{min}}$ が達成できるか：}
\begin{itemize}
\item \textbf{断熱膨張}：左室（高温 $T_A$）を準静的に膨張させると、気体は仕事をして温度が下がる。断熱なので $\delta Q = 0$、したがって $dS_A = 0$。
\item \textbf{断熱圧縮}：右室（低温 $T_B$）は圧縮され、仕事をされて温度が上がる。同様に $dS_B = 0$。
\item 仕切り壁を動かすだけなので、左室がした仕事 $=$ 右室がされた仕事であり、\textbf{系全体は孤立}（外界と仕事・熱のやりとりなし）。
\item 両室とも $dS = 0$ なので、全エントロピー $S_{\mathrm{f}} = S_{\mathrm{i}}$ を保ったまま両室の温度を揃えられる。
\end{itemize}

準静的に行えば可逆なので、$S_{\mathrm{f}} = S_{\mathrm{i}}$ を満たす最低温度 $T_{\mathrm{min}} = \sqrt{T_A T_B}$ が達成される。

最後に体積を元に戻す必要がある。温度が揃った後、透熱壁に交換して熱平衡を保ちながら準静的に仕切り壁を動かし、体積を $V_A$, $V_B$ に戻す。この過程は等温可逆なので $\Delta S = 0$ を維持でき、終温度は $T_{\mathrm{min}}$ のままである。

\textbf{問1との違い：}問1の「断熱壁を透熱壁に置き換える」操作は、熱が高温から低温へ一方向に流れる\textbf{不可逆過程}であり、$\Delta S > 0$ となる。そのため終温度は算術平均 $(T_A+T_B)/2$ となり、$T_{\mathrm{min}}$ より高い。

\paragraph{問6：最高温度 $T_{\mathrm{max}}$ とその操作}

\textbf{なぜ $T_{\mathrm{max}} = (T_A+T_B)/2$ か：}系は孤立しており、全内部エネルギー $U_{\mathrm{total}} = c R T_A + c R T_B$ は保存する。両室が同じ温度 $T$ になったとき、$U_{\mathrm{total}} = 2 c R T$ である。したがって $2 c R T = c R (T_A + T_B)$ より $T = (T_A+T_B)/2$ であり、$T$ はこの値を\textbf{超えられない}（エネルギーを外から加えられないため）。

\begin{equation}
\boxed{T_{\mathrm{max}} = \frac{T_A + T_B}{2}}.
\end{equation}

\textbf{$T_{\mathrm{max}}$ を達成する操作：}問1と同様、断熱壁を透熱壁に置き換えて熱平衡に達させる。高温側から低温側へ熱が流れ、両室の温度が算術平均で一致する。この過程では熱が一方向に流れるだけで、内部エネルギーを最大限に「均等配分」した状態になっており、これが達成しうる最高の共通温度である。

\textbf{なぜ $T_{\mathrm{max}}$ を超えられないか：}孤立系では外部からエネルギーを加えられない。両室の温度を $T$ に揃えるとき、$2cRT = cR(T_A + T_B)$ が成り立つので、$T = (T_A+T_B)/2$ が上限となる。$T_{\mathrm{min}}$ の場合と異なり、エントロピー増大則は $T_{\mathrm{max}}$ を制限しない（$\Delta S \ge 0$ は常に満たせる）。制限するのはエネルギー保存則である。
...
\part{2023年度 再試験}
\setcounter{section}{0}
\renewcommand{\theHsection}{2023retake.\arabic{section}}
\renewcommand{\theHsubsection}{2023retake.\arabic{section}.\arabic{subsection}}
...
\paragraph{問5の前：$W_N(M)$ が「$E$ ごとに状態数を数えて和を取る」でどう出てくるか}

分配関数は「すべての量子状態について $e^{-\beta E_i}$ を足す」で定義される。状態は量子数の組 $(m_1,\ldots,m_N)$ で指定され、$m_i = 0,1,2,\ldots$ であり、全エネルギーは $E = \hbar\omega(m_1+\cdots+m_N) = \hbar\omega M$（$M = m_1+\cdots+m_N$）である。足し算の順序を変えて、\textbf{まず全エネルギーが $E = \hbar\omega M$ であるような状態だけを集めて個数 $W_N(M)$ を数え、その $M$ ごとに $W_N(M)\,e^{-\beta\hbar\omega M}$ を足す}と
\begin{align*}
Z_N &= \sum_{\text{全状態}} e^{-\beta E_i}
= \sum_{M=0}^\infty \Bigl( \text{「$m_1+\cdots+m_N = M$ を満たす状態」の個数} \Bigr) \cdot e^{-\beta\hbar\omega M} \\
&= \sum_{M=0}^\infty W_N(M)\,e^{-\beta\hbar\omega M}.
\end{align*}
したがって $W_N(M)$ は\textbf{「$m_1+\cdots+m_N = M$ を満たす非負整数の組 $(m_1,\ldots,m_N)$ の個数」}そのものである。問題設定の「状態 $(m_1,\ldots,m_N)$」「全エネルギー $\hbar\omega M$」と、この「$M$ ごとに状態数を数えて和を取る」は同じ $Z_N$ を表しており、$W_N(M)$ はそのときの「エネルギー $E=\hbar\omega M$ を持つ状態の数」である。

\paragraph{問5：状態数 $W_N(M)$ とボール・箱の対応}

上で $W_N(M)$ =「$m_1+\cdots+m_N = M$ を満たす非負整数の組 $(m_1,\ldots,m_N)$ の個数」と分かった。これは、\textbf{大きさ $M$ の「エネルギーのかたまり」を $N$ 個の振動子に配分するやり方の数}と言い換えられる。振動子 $i$ に $m_i$ 個の「エネルギー量子」（$1$ 個あたり $\hbar\omega$）を割り当て、合計が $M$ になるようにする。\textbf{エネルギー量子どうしは区別しない}（どれを振動子1に渡すかで区別しない）し、\textbf{振動子は区別する}（1番目と2番目は別）。だから「$M$ 個の区別できないボールを $N$ 個の区別できる箱に分ける方法の数」と同一であり、その数は重複組合せ $\binom{M+N-1}{N-1}$ で与えられる。すなわち
\begin{equation}
...
$P(E) \propto W_N(M)\,e^{-\beta E}$（$E = \hbar\omega M$）が最大になる $M$ を求める。$f(M) = \ln W_N(M) - \beta\hbar\omega M$ の極大で $\partial f/\partial M = 0$ とおく。

\textbf{ステップ1：$\ln W_N(M)$ にスターリングの公式をあてはめる。}

問5より $W_N(M) = (M+N-1)!\,\big/\,((N-1)!\,M!)$ なので、
\[
\ln W_N(M) = \ln(M+N-1)! - \ln(N-1)! - \ln M!.
\]
スターリングの公式 $\ln n! \approx n\ln n - n$（$n \gg 1$ のとき）を、$n = M+N-1$、$n = N-1$、$n = M$ のそれぞれに用いると、
\begin{align}
\ln(M+N-1)! &\approx (M+N-1)\ln(M+N-1) - (M+N-1), \\
\ln(N-1)! &\approx (N-1)\ln(N-1) - (N-1), \\
\ln M! &\approx M\ln M - M.
\end{align}
したがって（$M \gg 1$、$N \gg 1$ の条件で）
\begin{align}
\ln W_N(M) &\approx (M+N-1)\ln(M+N-1) - (M+N-1) \notag \\
&\quad - (N-1)\ln(N-1) + (N-1) - M\ln M + M.
\end{align}

\textbf{ステップ2：$M,N \gg 1$ のときの近似。}

$M$ と $N$ が十分大きいとき、$M+N-1$ と $M+N$ の差や、$N-1$ と $N$ の差は相対的に小さい（例：$M=N=100$ なら $M+N-1=199$、$M+N=200$ で相対誤差は約 $0.5\%$）。この問題では $M$ で微分した結果に $\ln((M+N)/M)$ のような項だけが効くため、$\ln(M+N-1)$ を $\ln(M+N)$ で置き換えても $M,N\gg1$ で誤差は無視できる。同様に $(M+N-1)$ を $(M+N)$ と書いてよい。ステップ1（758--760行）の定数項は $-(M+N-1)+(N-1)+M=0$ なので、近似後も定数項は残さず 0 とするのが一貫している。そこで
\begin{equation}
\ln W_N(M) \approx (M+N)\ln(M+N) - (N-1)\ln(N-1) - M\ln M \quad (M,N\gg1).
\end{equation}

\textbf{ステップ3：$M$ で微分して極大条件。}

$M$ で微分する。$\frac{d}{dM}\bigl((M+N)\ln(M+N)\bigr) = \ln(M+N) + 1$、$\frac{d}{dM}(M\ln M) = \ln M + 1$ なので、
\begin{equation}
\frac{\partial}{\partial M}\ln W_N(M) \approx \ln(M+N) + 1 - (\ln M + 1) = \ln\frac{M+N}{M}.
\end{equation}
極大条件 $\frac{\partial f}{\partial M} = \frac{\partial}{\partial M}\ln W_N(M) - \beta\hbar\omega = 0$ より
\begin{equation}
\frac{M+N}{M} = e^{\beta\hbar\omega}
\quad \Rightarrow \quad
1 + \frac{N}{M} = e^{\beta\hbar\omega}
