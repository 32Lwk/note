...
この問題では、エントロピーが $S(U,V)$ の形で与えられている。温度は $1/T = (\partial S/\partial U)_V$ で定義されるので、$S$ を $U$ で微分すれば $T$ が求まる。問2・問3は断熱自由膨張と準静的断熱圧縮の組み合わせで、2024年度問題Iと同様に、$T'=T$（断熱自由膨張で温度不変）と $\Delta S = N R \ln(V'/V)$ が成り立つ。
...
熱力学の関係式 $1/T = (\partial S/\partial U)_V$ を用いる。$S = NR \ln[(U/N)^{3/2}(V/N)^2] + N S_0$ において、$\ln[(U/N)^{3/2}(V/N)^2] = \frac{3}{2}\ln(U/N) + 2\ln(V/N)$ なので $U$ で偏微分すると $\frac{\partial}{\partial U}\ln(U/N) = \frac{1}{U/N}\cdot\frac{1}{N} = \frac{N}{U}\cdot\frac{1}{N} = \frac{1}{U}$。よって
\begin{equation}
\frac{\partial S}{\partial U} = N R \cdot \frac{3}{2} \cdot \frac{1}{U}
= \frac{3NR}{2U}.
\end{equation}
（$N$ モル全体の $S$ を $U$ で微分しているので、$N$ が残る。$U$ は全内部エネルギーである。）したがって
\begin{equation}
\frac{1}{T} = \left(\frac{\partial S}{\partial U}\right)_V = \frac{3NR}{2U}
...
準静的断熱過程 $(T',V') \to (T'',V)$ ではエントロピーは一定なので $dS = 0$。$S(U,V) = NR\ln[(U/N)^{3/2}(V/N)^2] + NS_0$ において、$S$ が一定なら $(U/N)^{3/2}(V/N)^2$ が一定、すなわち $U^{3/2} V^2 = \mathrm{const}$ が成り立つ（断熱可逆過程の関係式）。$U = (3/2)N R T$ なので $U^{3/2} \propto T^{3/2}$ であり、$T^{3/2} V^2 = \mathrm{const}$ となる。したがって $(T')^{3/2} (V')^2 = (T'')^{3/2} V^2$。$T'=T$ より
\begin{equation}
...
\caption{問題Iの過程：$(T,V) \to (T',V')$（断熱自由膨張、$T'=T$）、$(T',V') \to (T'',V)$（準静的断熱圧縮）。本問では $T'' = T(V'/V)^{2/3}$。}
